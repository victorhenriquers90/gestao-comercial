...
/**
 * A bare command name (e.g. `vite`) is a `.cmd` shim in `node_modules/.bin`
 * on Windows, and `spawn` cannot exec those without going through a shell.
 * Route only that case through `cmd.exe`, passing the command and its args
 * as separate argv entries (not a joined string) so spawn's own Windows
 * quoting still applies — unlike `{ shell: true }`, which concatenates args
 * unescaped and breaks on anything with a space (e.g. `C:\Program Files\...`).
 * A command that already has an extension (an absolute `node.exe` path, as
 * in this file's own tests) is spawned directly, unchanged.
 */
...
function main(argv) {
  ...
  const child = spawn(target.file, target.args, { stdio: "inherit", env });
