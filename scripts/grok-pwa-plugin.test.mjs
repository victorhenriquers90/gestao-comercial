...
// An empty directory to use as `cwd` for the head-injection tests.
//
// `normalizeHeadContext` falls back to the *real* workspace on two paths: it
// reads the app identity when `ctx.site` is omitted, and `applyCustomCardFromFs`
// looks for `public/og.*`. Once this template became an actual app (named app,
// real public/og.jpg) both leaked in and these assertions started failing —
// the injector was right, the tests just weren't hermetic. Pinning `cwd` here
// (plus an explicit `site`) keeps them about the injector's own logic.
const NO_WORKSPACE = mkdtempSync(join(tmpdir(), "grok-pwa-hermetic-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", site: {}, cwd: NO_WORKSPACE });
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", { appName: "Solo", site: {}, cwd: NO_WORKSPACE });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", site: {}, cwd: NO_WORKSPACE });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", site: {}, cwd: NO_WORKSPACE });
