...
export const authMiddleware = createMiddleware({ type: "function" })
  .client(async ({ next }) => {
    ...
    const { getBearerToken, ensureCsrfCookie } = await import("./client");
    return next({
      sendContext: {
        bearerToken: getBearerToken() ?? undefined,
        csrfToken: ensureCsrfCookie() ?? undefined,
      },
    });
  })
  .server(async ({ next, context }) => {
    ...
    const { assertCsrfFromContext } = await import("./csrf.server");
    const { requireUserId } = await import("./verify.server");
    // Reject scripted cross-site/sibling requests before touching per-user data.
    assertSameSiteRequest();
    assertCsrfFromContext(context.csrfToken);
