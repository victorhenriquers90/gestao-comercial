...
import { CSRF_HEADER, ensureCsrfCookie } from "./csrf";
import {
  createOAuthState,
...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    onRequest(ctx) {
      ...
      if (csrf) ctx.headers.set(CSRF_HEADER, csrf);
      return ctx;
    },
  },
});
...
export async function signIn(
  ...
  if (!isAllowedOAuthProvider(providerId, GROK_PROVIDERS)) {
    throw new Error("Provedor de login não habilitado.");
  }
  ...
  const popup = inLivePreview() ? openSignInPopup(providerId, oauthState) : null;
  ...
  if (inLivePreview()) {
    ...
    const token = await waitForPopupToken(popup, oauthState);
 ...
 */
function openSignInPopup(providerId: string, state: string): Window | null {
  const origin = window.location.origin;
  const url = `${origin}/auth/popup?providerId=${encodeURIComponent(providerId)}&state=${encodeURIComponent(state)}`;
 ...
 */
function waitForPopupToken(popup: Window, expectedState: string): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      const parsed = parseOAuthPopupMessage(event.data, origin, event.origin, expectedState);
      if (!parsed) return;
      settle(parsed.token ?? null);
    };
