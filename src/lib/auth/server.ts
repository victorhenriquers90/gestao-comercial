...
// `void` only silences the floating-promise lint, not a runtime rejection —
// db.ts already logs the failure, so just join without letting it crash here too.
ensureDbReady().catch(() => {});
...
export const auth = betterAuth({
  ...
  // Custom hash/verify: timing-safe compare + dummy scrypt on sign-in miss
  // (see `password.server.ts`; the route wraps sign-in in the dummy pad).
